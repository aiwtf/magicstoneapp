...
export default function MagicStone({ isAbsorbing = false, progress = 0 }: { isAbsorbing?: boolean, progress?: number }) {
    const [isHovered, setIsHovered] = useState(false);

    // Calculate dynamic scale based on progress (0-100)
    // Max growth: 1.5x at 100% progress
    ...
    return (
        ...
        >
            ...
            <motion.div
                ...
                style={{ opacity: 0.2 + (progress / 500) }}
                animate={{
                    scale: isAbsorbing ? [1, 2, 1] : [1, 1.2, 1],
                }}
                transition={{
                    duration: isAbsorbing ? 0.5 : 4,
                    repeat: Infinity,
                    ease: "easeInOut",
                }}
            />
            ...
            >
                ...
                <motion.div
                    ...
                    transition={{ duration: isAbsorbing ? 0.5 : 3, repeat: Infinity, ease: "easeInOut" }}
                />
            </motion.div>
            ...
            >
                {isAbsorbing ? "ABSORBING..." : "Touch the Stone"}
            </motion.p>
        </div>
    );
}
