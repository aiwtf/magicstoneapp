...
export default function SoulResultDisplay({ data, stoneIndex }: SoulResultDisplayProps) {
    ...
    // Truth Protocol: Synchronization Level
    const syncLevel = data.synchronization?.level ?? 1;
    const syncRate = data.synchronization?.rate ?? 0;
    const syncReason = data.synchronization?.missing_data_reason ?? '';
    const isLocked = (field: unknown) => field === null || field === undefined;

    // Split Conflict if possible
    const tensionRaw = data.core_tension
        ? (typeof data.core_tension === 'string'
            ? data.core_tension
            : (data.core_tension?.conflict || "Unresolved"))
        : null;

    // Try to split by 'vs' or 'vs.' or 'VS'
    const tensionParts = tensionRaw ? tensionRaw.split(/ vs\.? /i) : [];
    const tensionLeft = tensionParts[0] || null;
    const tensionRight = tensionParts[1] || null;

    // Stone visual class based on sync level
    const stoneVisualClass = syncLevel === 1
        ? 'grayscale opacity-50'
        : syncLevel === 2
            ? 'grayscale-[50%] opacity-75'
            : '';
    ...
    return (
        <div className="relative w-full max-w-5xl mx-auto flex flex-col items-center gap-12 font-serif text-zinc-100 pb-20">
            ...
            {/* 1.5 Synchronization Status Bar */}
            <motion.div
                ...
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1 h-1 bg-zinc-800 rounded-full overflow-hidden">
                        <motion.div
                            ...
                            className={`h-full rounded-full ${syncLevel === 3 ? 'bg-emerald-400' : syncLevel === 2 ? 'bg-amber-400' : 'bg-zinc-600'
                                }`}
                        />
                    </div>
                    <span className="text-[10px] font-mono text-zinc-500">{syncRate}%</span>
                </div>
                {syncReason && syncLevel < 3 && (
            ...
            <div className="relative w-full grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-12 md:gap-16">
                ...
                >
                    ...
                    {tensionLeft ? (
                        ...
                        <span className="text-sm text-zinc-700 italic flex items-center gap-1 pr-6">
                            <Lock className="w-3 h-3" /> Fog Locked
                        </span>
                    )}
                </motion.div>
                ...
                <div className="relative w-full md:w-[500px] h-[500px] flex items-center justify-center">
                    ...
                    {/* Stone Image — visual grading by sync level */}
                    <motion.img
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        transition={{ duration: 1, delay: 0.5 }}
                        className={`w-full h-full object-contain drop-shadow-2xl z-10 transition-all duration-1000 ${stoneVisualClass}`}
                ...
                >
                    ...
                    {tensionRight ? (
                        ...
                        <span className="text-sm text-zinc-700 italic flex items-center gap-1 pl-6">
                            <Lock className="w-3 h-3" /> Fog Locked
                        </span>
                    )}
                </motion.div>
                ...
                <div className="md:hidden text-center col-span-1 space-y-2 mt-4">
                    ...
                    {tensionLeft ? (
                        ...
                        <div className="text-sm text-zinc-700 italic flex items-center justify-center gap-1">
                            <Lock className="w-3 h-3" /> Need deeper data to unlock
                        </div>
                    )}
                </div>
            ...
            >
                ...
                {/* Shadow Section — Fog Locked if depth_analysis is null */}
                {isLocked(data.depth_analysis) ? (
                    <div className="p-8 bg-black/40 border border-zinc-800/50 rounded-2xl text-center space-y-3">
                        <Lock className="w-5 h-5 text-zinc-700 mx-auto" />
                        <h4 className="text-sm text-zinc-600 uppercase tracking-widest">Fog Locked</h4>
                        <p className="text-[11px] text-zinc-700 italic max-w-sm mx-auto">
                            Need deeper conversation to unlock shadow analysis
                        </p>
                    </div>
                ) : (
                    <div
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-6 bg-zinc-900/30 border border-white/5 rounded-xl space-y-2">
                        ...
                        {isLocked(data.operating_system) ? (
                            <p className="text-sm text-zinc-700 italic flex items-center gap-1">
                                <Lock className="w-3 h-3" /> Fog Locked
                            </p>
                        ) : (
